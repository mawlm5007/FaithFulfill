...
- (BOOL)isiPhone14ProOrNewer {
    struct utsname systemInfo;
    uname(&systemInfo);
    NSString *deviceModel = [NSString stringWithCString:systemInfo.machine encoding:NSUTF8StringEncoding];
    // Check if the device model indicates iPhone 14 Pro or newer
    return ([deviceModel isEqualToString:@"iPhone14,3"] || // iPhone 14 Pro
            [deviceModel isEqualToString:@"iPhone14,4"] || // iPhone 14 Pro Max
            // Add more iPhone 15 models as needed
            );
}
...
- (void) isAvailableWithResolver:(RCTPromiseResolveBlock) resolve
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        currentDevice.proximityMonitoringEnabled = [self isiPhone14ProOrNewer]; // Enable only for iPhone 14 Pro and newer
...
RCT_EXPORT_METHOD(getUpdateInterval:(RCTResponseSenderBlock) cb) {
    NSLog(@"getUpdateInterval is not meaningful for a proximity sensor");
